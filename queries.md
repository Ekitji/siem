...
##### Look for PATHS pointing to User-Writable ones like C:\ProgramData\ OR C\:Users\ OR C:\Windows\Temp
...
##### Look for PATHS pointing to User-Writable ones like C:\ProgramData\* OR C:\Users\ OR C:\Windows\Temp in winlog.event_data.Details field.
...
#### Generic query - If needed add ProgramData, Users, C-root, Network shares etc to the query to narrow it down. Could also be interesting to query for **none-admin** executed scripts by modifying the query. It could be that no admin has still not logged in where that GPO is set and it could be a vulnerability when high privileged user logs in. Could also be that none admin scripts has plain text credentials.
```
...
> **Ref:**  https://offsec.blog/hidden-menace-how-to-identify-misconfigured-and-dangerous-logon-scripts/ check the Use case topics in link for detailed description of what to look for.
...
```
event.provider: "Microsoft-Windows-Security-Auditing" AND event.code: 5136 AND winlog.event_data.AttributeLDAPDisplayName: scriptPath
```
